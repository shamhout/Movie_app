...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void initState() {
    super.initState();
    avatarController = PageController(viewportFraction: 0.32, initialPage: selectedAvatar);
  }
    ...
    try {
      ...
      if (res.success) {
        if (res.data != null && res.data!.token != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(res.message ?? "account register complete! you need to login now!"),
              backgroundColor: Colors.green,
        ...
        _showErrorSnackBar(res.message ?? "error while register this account please try again later");
      }
    } catch (e) {
      if (!mounted) return;
      setState(() {
              ...
              children: [
                ...
                CustomTextFormField(
                  ...
                  suffixIcon: GestureDetector(
                    onTap: () => setState(() => isPasswordObscured = !isPasswordObscured),
                ...
                CustomTextFormField(
                  ...
                  suffixIcon: GestureDetector(
                    onTap: () => setState(() => isRePasswordObscured = !isRePasswordObscured),
                    child: isRePasswordObscured ? Image.asset(AppAssets.eyeoff) : const Icon(Icons.remove_red_eye_outlined, color: Colors.white),
                  ),
                ),
                SizedBox(height: height * .024),
                CustomTextFormField(
